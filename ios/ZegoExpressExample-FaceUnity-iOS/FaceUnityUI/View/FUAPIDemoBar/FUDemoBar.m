...
@interface FUDemoBar ()<FUFilterViewDelegate, FUBeautyViewDelegate,FUMakeUpViewDelegate, FUItemsViewDelegate>
...
//    _itemsView = [[FUItemsView alloc] initWithFrame:CGRectMake(0,-10, [UIScreen mainScreen].bounds.size.width, 160)];
    self.itemsView.delegate = self;
    [self.itemsView updateCollectionArray:@[@"background_test", @"bling", @"fengya_ztt_fu", @"hudie_lm_fu", @"touhua_ztt_fu", @"juanhuzi_lm_fu", @"mask_hat", @"yazui", @"yuguan"]];
    ...
    [self.itemsBtn setTitle:NSLocalizedString(@"道具", nil) forState:UIControlStateNormal];
    ...
    [self.filterBtn setTitle:NSLocalizedString(@"美妆", nil) forState:UIControlStateNormal];
...
//    }
    ...
    if (self.itemsBtn.selected) {
//        NSInteger selectedIndex = self.itemsView.selectedIndex;
...
-(void)makeupFilter:(NSString *)filterStr value:(float)filterValue{
    ...
    self.selectedFilter = filterStr;
    self.selectedFilterLevel = filterValue;
